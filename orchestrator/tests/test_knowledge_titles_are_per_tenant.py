...
class DieMigrationLegtBeideTeilweisenIndizesAnTests(unittest.TestCase):
    """Das Modell allein genuegt nicht: die laufende Anlage bekommt die Bedingung
    ueber die Migration. Weichen beide voneinander ab, ist der Test oben gruen und
    die Produktion trotzdem kaputt."""
...
class JedeTitelsucheIstAufDenBesitzerEingeschraenktTests(unittest.TestCase):
    """Formtest: kein `KnowledgeEntry.title == ...` ohne `user_id` im selben Aufruf.

    Das Lockern der DB-Bedingung ist nur dann sicher, wenn WIRKLICH JEDE Titelsuche
    auf den Besitzer eingeschraenkt ist. Zur Laufzeit haette eine uebersehene Stelle
    keine Spur hinterlassen — sie wuerde still den Eintrag eines fremden Mandanten
    treffen oder mit MultipleResultsFound abbrechen. Deshalb am Quelltext pruefen.
    """

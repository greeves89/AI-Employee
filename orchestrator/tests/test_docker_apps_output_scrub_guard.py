...
class McpEnvWriteOrderGuardTests(unittest.TestCase):
    """#502 ist in main ueber die REIHENFOLGE geloest, nicht ueber eine Sperre.

    Die drei `os.environ`-Schreibvorgaenge sind einzeln atomar, zusammen nicht. Wer
    `CUSTOM_MCP_SERVERS` zuerst schreibt, laesst einen Leser im Worker-Thread einen
    neuen Server ohne passenden Token sehen — Ergebnis: 401. Steht SERVERS zuletzt,
    ist der schaedliche Fall unmoeglich; der umgekehrte (alte SERVERS, neue Creds)
    ist harmlos.

    Ohne diesen Test kippt die Reihenfolge beim naechsten Umbau still zurueck.
    """

    def test_servers_is_written_last(self):
        from pathlib import Path
        src = (Path(__file__).resolve().parents[2] / "agent/app/main.py").read_text()
        block = src.split("Always refresh the in-process env", 1)[1].split("failed_names", 1)[0]
        pos = {name: block.index(f'os.environ["CUSTOM_MCP_{name}"]')
               for name in ("SERVERS", "AUTH", "HEADERS")}
        self.assertGreater(pos["SERVERS"], pos["AUTH"],
                           "CUSTOM_MCP_SERVERS muss NACH AUTH geschrieben werden")
        self.assertGreater(pos["SERVERS"], pos["HEADERS"],
                           "CUSTOM_MCP_SERVERS muss NACH HEADERS geschrieben werden")

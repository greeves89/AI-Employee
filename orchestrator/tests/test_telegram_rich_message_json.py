...
async def test_rich_message_is_forwarded_as_dict_not_string():
    ...
    assert rm["blocks"][0]["type"] == "section_heading"
    rk = captured["data"]["reply_markup"]
    assert isinstance(rk, dict), f"reply_markup must be a dict, got {type(rk).__name__}"
    # chat_id stays its declared type (int) so Telegram's JSON parser handles it normally.
...
async def test_draft_endpoint_also_forwards_as_dict():
    body = SendRichMessageRequest(
        chat_id=480455764,
        blocks=[{"type": "paragraph", "text": "draft chunk"}],
    )
    ...
    assert isinstance(captured["data"]["rich_message"], dict)
    assert captured["data"]["rich_message"]["blocks"][0]["text"] == "draft chunk"

...
_PLATFORM_SECTION = (
    ...
    "file'. You (the agent container) have NO docker yourself — do NOT run `docker`/`docker compose`; "
    "the PLATFORM discovers apps under `/workspace/projects/` and starts them (compose up) for the user "
    "via the orchestrator. The user reaches a running app through the app proxy, not a host port.\n\n"
